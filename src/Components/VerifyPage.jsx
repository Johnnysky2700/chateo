  ...
  return (
    <div className="flex flex-col min-h-screen justify-between p-6 bg-white dark:bg-black text-black dark:text-white">
      <div>
        ...
        <div className="mb-6" onClick={() => setShowKeypad(true)}>
          <PhoneInput
            country={'id'}
            value={phoneNumber}
      ...
      {showKeypad && (
        <div className="grid grid-cols-3 gap-4 text-2xl text-center mt-6 bg-[#F7F7FC] dark:bg-black p-4 absolute bottom-0 w-full left-0">
          {keypadNumbers.map((key, idx) => (
            <button
              key={idx}
              onClick={() => key && handleInput(key)}
              className="py-4 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-black dark:text-white"
            >
